...
export default function DataSourcesTab({ chatbotId }: DataSourcesTabProps) {
  ...
  useEffect(() => {
    if (existingDataSources && Array.isArray(existingDataSources) && existingDataSources.length > 0) {
      const mappedSources = existingDataSources.map((source: any) => ({
        ...
        status: source.processed ? 'processed' : 'pending',
      }));
  ...
  const processDataSource = async (source: Omit<DataSource, 'id'>) => {
    try {
      ...
      // If chatbot exists, save directly to database
      if (chatbotId) {
        apiUrl = `/api/chatbots/${chatbotId}/data-sources`;
        body = {
          type: source.type,
      ...
      setLocalDataSources(prev => prev.map(ds => 
        ds.name === source.name ? { ...ds, status: 'processed' as const } : ds
      ));
      ...
      <div className="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-lg p-4 mt-6">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h4 className="font-medium text-gray-900 mb-1">AI-Powered Knowledge Base</h4>
